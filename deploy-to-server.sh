#!/bin/bash
################################################################################
# SHADOW SEVEN — FULLSTACK DEPLOYMENT SCRIPT v4.0.0
...
$SSH_CMD "rm -rf ${REMOTE_PATH:?}/* ${REMOTE_PATH:?}/.* 2>/dev/null || true" || true
...
# Exclude node_modules and build artifacts to speed up transfer
rsync -avz \
...
$SSH_CMD "cd $REMOTE_PATH && npm install"
...
# Supabase API Gateway (through Nginx proxy at port 8000)
VITE_SUPABASE_URL=http://localhost:8000
VITE_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsY2l0YWd0aGp4bXZ5eWloamZmIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTAwMDAwMDAsImV4cCI6MTkyMDAwMDAwMH0.PLACEHOLDER_ANON_KEY
...
HEALTH_CHECK=$($SSH_CMD "curl -s -o /dev/null -w '%{http_code}' http://localhost:3002 2>/dev/null || echo '000'" || echo "000")

if [ "$HEALTH_CHECK" = "200" ]; then
    log_success "Application is running and responding"
else
    log_warning "Application returned status code: $HEALTH_CHECK (may need to wait a moment)"
fi

log_info "\nRetrieving application logs..."
$SSH_CMD "pm2 logs shadow-seven-ui --lines 5 --nostream"
...
echo -e "  1. Set up Nginx reverse proxy to port 3002"
echo -e "  2. Configure SSL/TLS certificates"
echo -e "  3. Update Supabase credentials in .env.production"
echo -e "  4. Connect to Nexus Core API on port 3001"
echo -e "  5. Test the full integration\n"

echo -e "${BLUE}Useful Commands:${NC}"
echo -e "  SSH to server:    ssh ${REMOTE_USER}@${REMOTE_HOST}"
echo -e "  View logs:        pm2 logs shadow-seven-ui"
echo -e "  Restart app:      pm2 restart shadow-seven-ui"
echo -e "  Stop app:         pm2 stop shadow-seven-ui"
echo -e "  Rebuild frontend: cd ${REMOTE_PATH} && npm run build && pm2 restart shadow-seven-ui\n"
